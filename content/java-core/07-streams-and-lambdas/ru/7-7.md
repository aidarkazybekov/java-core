...
## Tip
Use the NQ model as a rule of thumb: parallelize when N (element count) * Q (cost per element operation) > 10,000. For cheap operations like simple filtering, you need millions of elements to justify the fork/join overhead.
...
## Interview
### [7-7-q0 | junior] When should you use parallel streams, and when should you avoid them?
...
### [7-7-q1 | mid] Why is the common ForkJoinPool problematic for parallel streams, and how can you mitigate it?
The common ForkJoinPool is shared JVM-wide. A slow parallel stream blocks threads other tasks need, causing thread starvation. For example, a parallel stream doing I/O can block all common pool threads, stalling unrelated CompletableFuture.supplyAsync() calls. Mitigation: submit the stream operation to a custom ForkJoinPool with a controlled thread count. However, this is an unofficial workaround -- the Stream API has no formal custom pool support. Another approach is to size the common pool with -Djava.util.concurrent.ForkJoinPool.common.parallelism.

### [7-7-q2 | senior] Explain how Spliterator characteristics affect parallel stream performance. Why does ArrayList parallelize better than LinkedList?
